...
ResidualModelFlyHighExpTpl<Scalar>::ResidualModelFlyHighExpTpl(boost::shared_ptr<StateMultibody> state,
                                                               const std::size_t nu,
...
template <typename Scalar>
ResidualModelFlyHighExpTpl<Scalar>::~ResidualModelFlyHighExpTpl() {}

template <typename Scalar>
void ResidualModelFlyHighExpTpl<Scalar>::calc(const boost::shared_ptr<ResidualDataAbstract> &data,
    ...
    d->h_exp = exp(-d->pinocchio->oMf[frame_id_].translation()[2] * gamma_);
...
template <typename Scalar>
void ResidualModelFlyHighExpTpl<Scalar>::calcDiff(const boost::shared_ptr<ResidualDataAbstract> &data,
                                                  const Eigen::Ref<const VectorXs> &,
                                                  const Eigen::Ref<const VectorXs> &) {
    ...
    d->Rx = d->h_exp * d->dv_dx.topRows(2);
    d->Rx.leftCols(nv_) += - d->r * gamma_ * d->dv_dx.rightCols(nv_).row(2);
    STOP_PROFILER("ResidualModelFlyHighSqr::calcDiff");
}

template <typename Scalar>
boost::shared_ptr<ResidualDataAbstractTpl<Scalar> > ResidualModelFlyHighExpTpl<Scalar>::createData(
        DataCollectorAbstract *const data) {
    return boost::allocate_shared<Data>(Eigen::aligned_allocator<Data>(), this, data);
}

template <typename Scalar>
pinocchio::FrameIndex ResidualModelFlyHighExpTpl<Scalar>::get_frame_id() const {
    return frame_id_;
}

template <typename Scalar>
Scalar ResidualModelFlyHighExpTpl<Scalar>::get_gamma() const {
    return gamma_;
}
